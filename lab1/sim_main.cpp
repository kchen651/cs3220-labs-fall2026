...
int main(int argc, char** argv, char** env) {
    ...
    while (timestamp < RUN_CYCLES) {      
        ...
        if (clk_transition && dut->clk) {
            ...
            int  op_WB    = (int)dut->pipeline->my_WB_stage->WB_counters[3];

            // End of program: stop before printing, so the sentinel word does
            // not show up in the trace as a retired instruction.
            if (valid_WB && op_WB == INVALID_I_OP) {
            #ifdef VCD_OUTPUT
                trace->dump(timestamp);
            #endif
                std::cout << "-- stopped at PC=0x" << std::hex
                          << (int)dut->pipeline->my_WB_stage->WB_counters[1]
                          << std::dec << " (end of program) --" << std::endl;
                break;
            }
